...
object Auth {
  def make[F[_]: MonadThrow](
      ...
      crypto: Crypto): Auth[F] = new Auth[F] {
    ...
    override def login(username: UserName, password: Password): F[JwtToken] =
      users.find(username).flatMap {
        ...
        case Some(user) => redis.get(username.show).flatMap {
          ...
          case None => tokens.create.flatTap { t =>
...
object UsersAuth {
  def common[F[_]: Functor](redisCommands: RedisCommands[F, String, String]): UsersAuth[F, CommonUser] = new UsersAuth[F, CommonUser] {
