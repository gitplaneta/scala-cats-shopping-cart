...
object ShoppingCart {
  def make[F[_]: GenUUID: MonadThrow](
    ...
    override def delete(userId: UserId): F[Unit] = redis.del(userId.show).void

    override def removeItem(userId: UserId, itemId: ItemId): F[Unit] = redis.hDel(userId.show, itemId.show).void
